...
module.exports = {
  addNew: async (req, res) => {
    try {
      const doc = new BookCategoryModel(req.body);
  ...
  updateOne: async function (req, res) {
    try {
      const doc = await BookCategoryModel.findById(req.params.id);
      if (!doc) {
        return res.status(404).json({ message: "Ma'lumot topilmadi" });
      }
      const result = await BookCategoryModel.findByIdAndUpdate(
        req.params.id,
        req.body
      );
      if (!result) {
  ...
  deleteOne: async function (req, res) {
    try {
      ...
      const result = await BookCategoryModel.findByIdAndDelete(req.params.id);
